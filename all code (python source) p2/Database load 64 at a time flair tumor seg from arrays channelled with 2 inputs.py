    ...
    for j in range(0,int(num_files/chunk_size)+1):
        ...
        for i in range(set_size*j,set_size*(j+1)):
            try:
                ...
                channeled_image = np.stack([image,image],axis = 3)
                channeled_image[:,:,:,1] = 0
                channeled_image[:,:,:,1][mask > 0] = 1
                print("blabla", channeled_image[:,:,:,1].shape)
                print("blabla", np.unique(channeled_image[:,:,:,1]))
                print(channeled_image.shape)
                
                images.append(channeled_image)
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays",
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays/Images and Masks For Tumor Segmentation")
