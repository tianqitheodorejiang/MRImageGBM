    ...
    for set_ in os.listdir(path):
        try:
            if int(set_) not in bad_indexes:
                ...
                only_brain = image.copy()
                only_brain[mask < 1] = 0

                median = find_median_grayscale(only_brain)

                image = image/(median/0.5)
                ...
                image[big_mask > 0] *= ((brain_median/100)/median)
                #t11[big_mask == 0] *= (0.05/median_skull)

                image = blur(image, (blur_amt/10))
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/t1 to flair arrays",
