...
def change_contrast(array, factor):
    new_array = array.copy()*255/np.max(array)

    for n,image in enumerate(new_array):

        bw = Image.fromarray(np.uint8(image))


        enhancer = ImageEnhance.Contrast(bw)

        decontrasted = enhancer.enhance(factor)

        final = np.asarray(decontrasted)
...
def get_files(path, img_id = "T1", mask_id = "aseg", save_folder_path = None, chunk_size = 64, image_size = 128):
    ...
    for SET in os.listdir(path):
        if "OAS" in SET:
            ...
            decontrasted_array = change_contrast(decontrasted_array, 0.2)
            ...
            median = find_median_grayscale(only_brain)
            ...
            decontrasted_array *= (0.25/median) 

            t1 = mask_off(decontrasted_array, t1, big_mask)
            ...
            image_data1 = skimage.transform.rescale(t1, (z_zoom, y_zoom, x_zoom))
            ...
            image_data = np.stack([np.stack([t1], axis = 3)])
            ...
            circled = circle_highlighted(t1, mask, 0.8)
