...
input_path = "C:\Users\JiangQin\Documents\python\ct to tumor identifier project\raw ct files\TCGA-GBM"
output_path = "C:\Users\JiangQin\Documents\python\ct to tumor identifier project\raw ct files\TCGA-GBM DATA"

if not os.path.exists(output_path):
    os.mkdirs(output_path)

folder_list = get_folder_list(input_path, "flair")

print("folders found: " + str(len(folder_list)))
added_folders = 0
for folder in folder_list:
    folder_name = ntpath.basename(folder)


    each_ct_path = output_path + "/" + folder_name
    if not os.path.exists(each_ct_path):
        os.mkdir(each_ct_path)
        added_folders += 1

    for file in os.listdir(folder):
        
        current_path = folder + "/" + file
        new_path = each_ct_path + "/" + file
        
        shutil.move(current_path, new_path)
        


print("new folders added: " + str(added_folders))
