...
area_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Tumor Seg Model 5/Model 3"
...
output_image_path = "/home/jiangl/Documents/python/ct to tumor identifier project/image ct  visualizations/Model Testing/"
output_image_path_Seg = "/home/jiangl/Documents/python/ct to tumor identifier project/image ct  visualizations/brain 1"


models_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS"
...
images_path = os.path.join(models_path + "/bran seg images")
if os.path.exists(images_path):
    shutil.rmtree(images_path)
os.mkdir(images_path)

model_list = []
for thing in os.listdir(models_path):
    if not os.path.isdir(models_path + "/" + thing):
        model_list.append(thing)
...
if sortable:
    ...
    for model_number in model_numbers:
        model_list.append("Model " + str(model_number))
...
for model in model_list:
    ...
    model_images_path = images_path + "/" + model
    ...
    brain_seg = keras.models.load_model(model_path)
    ...
    brain_mask_3d_array = np.squeeze(brain_mask1.copy()[0], axis = 3)
    ...
    circled_3d_array = np.squeeze(circled.copy()[0], axis = 3)

    multi_array = []

    for Slice in brain_mask_3d_array:
        multi_array.append(Slice)
    for Slice in circled_3d_array:
        multi_array.append(Slice)

    write_images(np.stack([np.stack([np.stack(multi_array)], axis = 3)]), model_images_path)
