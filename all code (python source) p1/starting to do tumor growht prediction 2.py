...
def cast_bounding(array, wanted_bounds, current_bounds):
    ...
    #print("current bounds: " + str(current_bounds))
    ...
    #print([left_rescaled,right_rescaled,low_rescaled,high_rescaled,shallow_rescaled,deep_rescaled])

    translate_x = right_wanted-right_rescaled
    translate_y = high_wanted-high_rescaled
    translate_z = deep_wanted-deep_rescaled
    #print("translation: " + str((translate_z,translate_y,translate_x)))
    ...
    #print("translated bounds: " + str(locate_bounds(np.stack([np.stack([translated], axis = 3)]))))
...
def cast_with_rotation(array, reference_array):
    binary = np.squeeze(array.copy()[0], axis = 3)
    binary[:] = 0
    binary[np.squeeze(array.copy()[0], axis = 3) > 0.1] = 1

    flat_z_axis_average = binary[0].copy()
    

    for image in binary:
        flat_z_axis_average[image > 0] = 1

    wanted_flat = imutils.rotate_bound(flat_z_axis_average, 270)

    y_zoom = image_size/wanted_flat.shape[0]
    x_zoom = image_size/wanted_flat.shape[1]

    wanted_flat = skimage.transform.rescale(wanted_flat, (y_zoom, x_zoom))

    plt.imshow(wanted_flat)
    plt.show()

    
    wanted_flat[wanted_flat > 0] = 1

    #print(np.stack([wanted_flat]).shape)

    wanted_flat_bounds = locate_bounds(np.stack([np.stack([np.stack([wanted_flat,wanted_flat])], axis = 3)]))

    #print(wanted_flat_bounds)
    ...
    for i in range(1,360):
        ...
        rotate_flat = imutils.rotate_bound(flat_z_axis_average,i)
        ...
        #print(np.count_nonzero(eliminated))
        #print("")

    rotation = non_zeros.index(np.min(non_zeros))+1
...
bounds = locate_bounds(original_array)

original_array = cast_bounding(original_array, [9,117,2,126,0,127], bounds)

cast_with_rotation(original_array, original_array)


adapted = adaptive_threshold(original_array,25,25,1)
...
brain_seg = keras.models.load_model(brain_seg_model)

brain_mask = brain_seg.predict(original_array)

binary_brain = binarize(brain_mask, 0.3)



brain_seg_array[binary_brain == 0] = 0
...
precise_prediction = precise_model.predict(brain_seg_array)

area_prediction[brain_seg_array < 0.1] = 0
precise_prediction[brain_seg_array < 0.1] = 0

area_tumor_binary = binarize(area_prediction, 0.1)
area_tumor_binary = brain.erode_down(area_tumor_binary, 2)



precise_tumor_binary1 = binarize(precise_prediction, 0.02)

precise_tumor_binary = combine_zeros(area_tumor_binary, precise_tumor_binary1)

precise_tumor_binary = touching_island(precise_tumor_binary, precise_tumor_binary1)

precise_tumor_binary = kill_smaller_islands(precise_tumor_binary, 1, 500)


circled = circle_highlighted(original_array, precise_tumor_binary, 1)

write_images(circled, output_image_path_Seg)



brain_seg = keras.models.load_model(area_model_path)

brain_mask = brain_seg.predict(original_array)

binary_brain = binarize(brain_mask, 0.1)



brain_seg_array[binary_brain == 0] = 0

tumor_parts = combine_zeros(brain_seg_array, adapted)

binary = binarize(tumor_parts, 0.5)

erode = brain.erode_down(binary, 2)



correct1 = touching_island(precise_tumor_binary, erode)

correct = branch(correct1, adapted, 10)

combined = combine_zeros(correct, tumor_parts)



circled = circle_highlighted(original_array, correct, 1)
