...
def get_files(path, img_id = "original array", mask_id = "rough brain", save_folder_path = None, chunk_size = 64, image_size = 128):
    images = []
    masks = []
    ...
    for path, dirs, files in os.walk(path, topdown=False):
        for file in files:
            if img_id in file.lower() or mask_id in file.lower():
                num_files += 1
    ...
    for path, dirs, files in os.walk(path, topdown=False):
        for file in files:
            ...
            elif mask_id in file.lower():
                masks_path = os.path.join(path, file)
                h5f = h5py.File(masks_path,'r')
                mask = h5f['all_data'][:]
                h5f.close()
                mask = np.squeeze(mask.copy()[0], axis = 3)
...
if data ==  main:
    get_files("/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/TCGA GBM DATA",
