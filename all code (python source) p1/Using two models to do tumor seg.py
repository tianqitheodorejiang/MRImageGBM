...
precise_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/True area/Model 8"
area_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/True area/Model 6"
...
file = get_folder_path("/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/Brain-Tumor-Progression", 16, "flair")
...
binary_pre = binarize(precise_prediction, 0.1)



circled = circle_highlighted(original_array, binary_pre, 1)


write_images(circled, output_image_path)

"""
adapted_brain_seg = adaptive_threshold(brain_seg_array, 15, 15)

tumor_binary_threshed = binarize(adapted_brain_seg, 0.7)

tumor_threshed_array[tumor_binary_threshed == 0] = 0



write_images(tumor_threshed_array, output_image_path_Seg)




adapted = adaptive_threshold(array, 15, 15)

binary1234 = binarize(adapted, 0.5)

dilate = dilate_up(binary1234, original_array, 2)

com123 = combine_zeros(dilate, adapted)
...
write_images(array, output_image_path_Seg)

"""
area_model = keras.models.load_model(area_model_path)
precise_model = keras.models.load_model(precise_model_path)

precise_prediction = precise_model.predict(array41)
area_prediction = area_model.predict(array41)

precise_prediction[array == 0] = 0
area_prediction[array == 0] = 0

binary_pre = binarize(precise_prediction, 0.1)
binary_area = binarize(area_prediction, 0.1)

correct = touching_island(binary_pre, com123)


#original_array[correct> 0] = 1

#com = combine_zeros(adapted, correct)
