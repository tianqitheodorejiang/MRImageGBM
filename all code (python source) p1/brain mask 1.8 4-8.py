...
#binary_brain_area2 = dilate_up(binary_brain_area2, blank, 5)
...
new_array1 = brain_seged/(median/0.04)
new_array2 = brain_seged/(median/0.2)

brain_mask1 = brain_seg.predict(new_array1)
brain_mask2 = brain_seg.predict(new_array2)

binary_brain_area11 = binarize(brain_mask1, 0.3)
binary_brain_area12 = binarize(brain_mask2, 0.3)

binary_brain_area111 = combine_zeros(binary_brain_area11, binary_brain_area12)
...
binary_brain_area21 = binarize(brain_mask1, 0.8)
binary_brain_area22 = binarize(brain_mask2, 0.8)

binary_brain_area2 = combine_zeros(binary_brain_area21, binary_brain_area22)

binary_brain_area22 = combine_zeros(binary_brain_area2, binary_brain_area111)


mask_upscaled = skimage.transform.rescale(np.squeeze(binary_brain_area22.copy()[0], axis = 3), (backscale_factor, backscale_factor, backscale_factor))
...
brain_mask = combine_zeros(brain_mask_area_rescaled, adapted)

brain_mask = combine_white_binary(brain_mask, brain_mask_precise_rescaled)


brain_mask = kill_small_islands(brain_mask, 2)

'''
brain_mask = dilate_up(brain_mask, blank, 2)

brain_mask = combine_zeros(brain_mask, adapted)
...
circled = circle_highlighted(original_unscaled_array, brain_mask_precise_rescaled, 0.8)
write_images(circled, output_image_path_Seg)


write_images(original_unscaled_array, output_image_path_Seg2)
