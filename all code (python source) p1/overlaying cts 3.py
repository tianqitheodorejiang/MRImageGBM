...
def bounds_scale_2d(array, wanted_bounds, current_bounds):
    ...
    rescaled_array = skimage.transform.rescale(array, (everage_zoom, average_zoom))
    ...
    print("u dumb boi: " + str(locate_bounds_2d(array)))
    print("u dumb boi2: " + str(locate_bounds_2d(translated)))
    print("u dumb boi2: " + str(wanted_bounds))


    return translated

def scale_casting_with_rotation(array3d, reference, rotation):
    '''
    back_rotated = array3d.copy()
    for n,image in enumerate(back_rotated):
        back_rotated[n] = imutils.rotate(image,-rotation)
    
    back_rotated_reference = reference.copy()
    for n, image in enumerate(back_rotated_reference):
        back_rotated_reference[n] = imutils.rotate(image,-rotation)

    print(back_rotated.shape)

    '''
    bounding_casted = cast_bounding(np.stack([np.stack([array3d], axis = 3)]),locate_bounds(np.stack([np.stack([reference], axis = 3)])), locate_bounds(np.stack([np.stack([array3d], axis = 3)])))


    rotation_original = np.squeeze(bounding_casted.copy()[0], axis = 3).copy()
    '''
    for n,image in enumerate(rotation_original):
        rotation_original[n] = imutils.rotate(image,rotation)
    '''
...
def correct_rotation_z_axis(array, reference_array):
    ...
    for i in range(0,360):
        print(i)
        eliminated = binary_reference.copy()
        rotated_binary = []
        for n,image in enumerate(binary_array.copy()):
            rotated_binary.append(imutils.rotate_bound(image.copy(),i))
        rotated_binary = np.stack(rotated_binary)
        
        z_zoom = image_size/rotated_binary.shape[0]
        y_zoom = image_size/rotated_binary.shape[1]
        x_zoom = image_size/rotated_binary.shape[2]

        rotate_binary = skimage.transform.rescale(rotated_binary, (z_zoom, y_zoom, x_zoom))

        rotate_binary[rotate_binary > 0] = 1

        casted = scale_casting_with_rotation(rotate_binary, binary_reference, i)
        #plt.imshow(casted[50])
        #plt.show()
        
        eliminated[casted > 0] = 0

        #plt.imshow(eliminated[50])
        #plt.show()
...
original_array12 = np.stack([np.stack([original_array1], axis = 3)])
...
write_images(original_array12, output_image_path_Seg)
...
corrected = correct_rotation_z_axis(original_array, original_array12)
