...
def get_files(path, start_index = 0, finish_index = False, img_id = "flair", save_folder_path = None, chunk_size = 64, image_size = 128):
    ...
    for path, dirs, files in os.walk(path, topdown=False):
        for dir_ in dirs:
            if img_id in dir_.lower():
                imgs.append(os.path.join(path, dir_))

    if not finish_index:
        finish_index = len(imgs)
    for i in range(start_index, finish_index):
        try:
            print("\n\n" + str(i) + "\n\n")
            path = imgs[i]
            image_size = 128
            brain = highlight_ct(path)
            brain.load_scan()
            ...
            save_array(original_array, path + "/original array.h5")
            ...
            new_array1 = brain_seged/(median/0.15)


            brain_mask1 = brain_seg.predict(new_array1)

            binary_brain_area12 = binarize(brain_mask1, 0.3)

            save_array(binary_brain_area12, path + "/rough brain seg.h5")             
        except:
            print("FAILED")
...
if data ==  main:
    get_files("/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/TCGA GBM DATA", img_id = "flair", start_index = 104, 
