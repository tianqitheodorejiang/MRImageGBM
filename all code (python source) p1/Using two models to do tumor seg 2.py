...
class highlight_ct:
    ...
    def erode_down(self, array, size):
        binary = np.squeeze(array.copy()[0], axis = 3)
        masked = np.squeeze(self.resampled_array.copy()[0], axis = 3)
...
file = get_folder_path("/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/Brain-Tumor-Progression", 9, "flair")
...
white = binarize(brain_seg_array, 0.5)

white_threshed = brain.erode_down(white, 1)


circled = circle_highlighted(brain_seg_array, white_threshed, 1)

write_images(circled, output_image_path)


print("median: " + str(brain.find_median_grayscale(brain_seg_array)))


precise_model = keras.models.load_model(area_model_path)

precise_prediction = precise_model.predict(brain_seg_array)

precise_prediction[brain_seg_array < 0.1] = 0

precise_tumor_binary = binarize(precise_prediction, 0.1)

correct = touching_island(precise_tumor_binary, white_threshed)
