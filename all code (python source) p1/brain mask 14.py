...
binary_brain_area1 = combine_zeros(binary_brain_area11, binary_brain_area13)
...
precise_tumor_binary = binarize(precise_tumor_prediction, 0.1)
...
tumor_deleton = dilate_up(precise_tumor_binary_rescaled, blank, 10)

def_no_tumor = brain.invert(tumor_deleton)

brain_wo_tumor = combine_zeros(brain_seged_extra, def_no_tumor)

brain_wo_tumor = kill_small_islands(brain_wo_tumor, 2)





brain_max = np.max(brain_wo_tumor)

print("brain max:", brain_max)

brain_thresh = brain.threshold_scans(original_unscaled_array, 0.05, brain_max, 0)





circled = circle_highlighted(original_unscaled_array, brain_wo_tumor, 0.8)


write_images(brain_wo_tumor, output_image_path)
