...
precise_model_path1 = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Tumor Seg Model 13/Model 41"
...
brain_seg_model_precise = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Brain Seg 4/Model 2"
...
binary_brain_area1 = binarize(brain_mask, 0.2)

brain_seg_array_area = original_array.copy()

brain_seg_array_area[binary_brain_area1 == 0] = 0
...
binary_brain_area = binarize(brain_mask, 0.9)

binary_brain_area = dilate_up(binary_brain_area, blank, 2)

binary_brain_final = combine_white_binary(binary_brain_area1, binary_brain_area)

brain_seg_precise = original_array.copy()

brain_seg_precise[binary_brain_area == 0] = 0


brain_seg_area = original_array.copy()

brain_seg_area[binary_brain_final == 0] = 0



brain_seg_area = brain_seg_area/np.max(brain_seg_array_area)


tumor_seg_area1 = keras.models.load_model(area_model_path1)

tumor_mask_area1 = tumor_seg_area1.predict(brain_seg_area)

tumor_seg_area2 = keras.models.load_model(area_model_path2)

tumor_mask_area2 = tumor_seg_area2.predict(brain_seg_area)

tumor_seg_area3 = keras.models.load_model(area_model_path3)

tumor_mask_area3 = tumor_seg_area3.predict(brain_seg_area)

tumor_seg_precise = keras.models.load_model(precise_model_path1)

tumor_mask_precise1 = tumor_seg_precise.predict(brain_seg_area)




binary_tumor_area1 = binarize_max(tumor_mask_area1, 0.3)

binary_tumor_area2 = binarize_max(tumor_mask_area2, 0.95)

binary_tumor_area3 = binarize_max(tumor_mask_area3, 0.95)

binary_tumor_precise1 = binarize_max(tumor_mask_precise1, 0.25)


binary_area = combine_white_binary(binary_tumor_area1, binary_tumor_area2)
binary_area = combine_white_binary(binary_area, binary_tumor_area3)


binary_final = combine_zeros(binary_tumor_precise1, binary_area)


binary_tumor_area = original_array.copy()

binary_tumor_area[binary_area == 0] = 0




circled = circle_highlighted(original_array, binary_final, 1)
circled = circle_highlighted(circled,  binary_brain_area1, 0.5)
