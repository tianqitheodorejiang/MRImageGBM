...
def adaptive_threshold(array, course, precise, blur_precision = 0):
    ...
    for image in blurred:
        thresh = cv2.adaptiveThreshold(image.astype('uint8'), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, course, 2)
        thresh2 = cv2.adaptiveThreshold(image.astype('uint8'), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, precise, 2)
...
binary_brain_area1 = binarize(brain_mask1, 0.35)
...
binary_brain_area1 = dilate_up(binary_brain_area1, blank, 2)

binary_brain_area13 = brain.erode_down(binary_brain_area1, 2)
...
brain_seged[binary_brain_area1 == 0] *= 0.1
...
print("divide value:", median/0.2)

new_array1 = original_array/(median/0.05)
new_array12 = original_array/(median/0.2)
new_array2 = original_array/(median/0.05)

brain_mask2_precise1 = brain_seg_precise1.predict(new_array1)
brain_mask2_precise12 = brain_seg_precise1.predict(new_array12)
brain_mask2_precise2 = brain_seg_precise2.predict(new_array2)




binary_brain_area11 = binarize(brain_mask2_precise1, 0.99)
binary_brain_area12 = binarize(brain_mask2_precise12, 0.99)
binary_brain_area2 = binarize(brain_mask2_precise2, 0.8)




binary_brain_area1 = combine_zeros(binary_brain_area11, binary_brain_area2)
binary_brain_area12 = combine_zeros(binary_brain_area12, binary_brain_area2)
binary_brain_area13 = combine_zeros(binary_brain_area1, binary_brain_area13)



brain_seged = original_array.copy()

brain_seged[binary_brain_area12 == 0] = 0


brain_seged = brain_seged/(median/0.2)

median = find_median_grayscale(brain_seged)
...
precise_tumor_binary = binarize(precise_tumor_prediction, 0.5)


mask_upscaled = skimage.transform.rescale(np.squeeze(binary_brain_area13.copy()[0], axis = 3), (backscale_factor, backscale_factor, backscale_factor))
...
mask_upscaled = skimage.transform.rescale(np.squeeze(precise_tumor_binary.copy()[0], axis = 3), (backscale_factor, backscale_factor, backscale_factor))
...
precise_tumor_binary_rescaled = np.stack([np.stack([mask_unscaled], axis = 3)])


original_unscaled_array = original_unscaled_array/(median/0.2)

brain_seged_extra = original_unscaled_array.copy()

brain_seged_extra[brain_mask_area_rescaled == 0] = 0


def_not_brain = brain.invert(brain_seged_extra)

adapted = adaptive_threshold(brain_seged_extra, 31, 51)

precise_tumor_binary_rescaled = combine_zeros(precise_tumor_binary_rescaled, adapted)

non_tumor = brain.invert(precise_tumor_binary_rescaled)


non_tumor_brain = combine_zeros(brain_seged_extra, non_tumor)

actual_adapted = adaptive_threshold(non_tumor_brain, 51, 51, 1)

actual_adapted = combine_white_binary(actual_adapted, precise_tumor_binary_rescaled)


actual_adapted = kill_small_islands(actual_adapted, 1)





no_brain = brain.invert(brain_mask_precise_rescaled)

blank = original_unscaled_array.copy()

blank[:] = 1

adapted = adaptive_threshold(original_unscaled_array, 51, 101)

no_brain = combine_zeros(no_brain, adapted)

no_brain = combine_white_binary(no_brain, def_not_brain)

no_brain = kill_small_islands(no_brain, 1)



no_brain_or_tumor = combine_zeros(no_brain, non_tumor)





new_brain = brain.invert(no_brain_or_tumor)

adapted = adaptive_threshold(new_brain , 51, 101, 1)

#adapted = combine_zeros(adapted, brain_mask_area_rescaled)

new_brain = kill_small_islands(adapted, 3)

threshed3 = brain.threshold_scans(original_unscaled_array, 0.05, 255, 0)





circled = circle_highlighted(original_unscaled_array, adapted, 0.8)


write_images(circled, output_image_path)
#write_images(brain_prob_precise_rescaled, output_image_path_Seg)

#brain.generate_stl(threshed3.T, "/home/jiangl/Documents/python/ct to tumor identifier project/3d stl ct visualizations", "brain new.stl", 1)
