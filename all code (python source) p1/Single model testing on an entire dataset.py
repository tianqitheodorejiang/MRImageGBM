...
model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Tumor Seg Model 13/Model 41"
database_path = "/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/Brain-Tumor-Progression"
...
def get_folder_list(path, img_id):
    pathes = []
    for path, dirs, files in os.walk(path, topdown=False):
        for dir_ in dirs:
            if img_id in dir_.lower():
...
images_of_model_path = images_path + "/" + model_name
if os.path.exists(images_of_model_path):
    shutil.rmtree(images_of_model_path)
os.mkdir(images_of_model_path)

data_list = get_folder_list(database_path, "flair")


for n,ct_path in enumerate(data_list):
    if n % 2 == 0:
        
        output_images_path = images_of_model_path + "/" + "Set " + str(n)
        ...
        binary = binarize(tumor_mask, 0.3)


        circled = circle_highlighted(original_array, binary, 1)


        write_images(circled, output_images_path)
