...
precise_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/True area/Model 9"
area_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/True area/Model 5"
...
class highlight_ct:
    ...
    #def remove_outliers(self, array, distance_max = 1):
...
original_array = original_array/(np.max(original_array)*1.2)
...
area_tumor_binary = binarize(area_prediction, 0.01)


white_bumped_up[area_tumor_binary == 0] *= 0.2


brain_seg_array = original_array.copy()


brain_seg1 = keras.models.load_model(precise_model_path)

brain_mask = brain_seg1.predict(white_bumped_up)

area_binary_brain = binarize(brain_mask, 0.2)

circled = circle_highlighted(original_array, area_binary_brain, 1)

write_images(white_bumped_up, output_image_path_Seg)


com = combine_zeros(area_binary_brain, adapted)

erode = brain.erode_down(com, 2)


precise_binary_brain = binarize(brain_mask, 0.5)


correct = touching_island(precise_binary_brain, erode)

correct = dilate_up(correct, original_array, 5)

binary1 = binarize(correct, 0.03)
...
brain_seg2 = keras.models.load_model(brain_seg_model)

brain_mask = brain_seg2.predict(white_bumped_up)
...
erode = brain.erode_down(com, 3)


precise_binary_brain = binarize(brain_mask, 0.4)
...
binary2 = binarize(correct, 0.01)

com = combine_zeros(binary1,binary2)

killed = kill_smaller_islands(com, 0, 2500)





brain_seg_array[killed == 0] = 0
...
erode = brain.erode_down(combined, 1)
...
circled = circle_highlighted(original_array, combined, 1)
