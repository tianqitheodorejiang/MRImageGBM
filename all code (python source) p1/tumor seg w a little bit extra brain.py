...
mask_rescaled = np.stack([np.stack([mask_unscaled], axis = 3)])
...
tumor_mask_rescaled = np.stack([np.stack([mask_unscaled], axis = 3)])
...
tumor_seged = original_unscaled_array.copy()

tumor_seged[mask_rescaled == 0] = 0

tumor_median = find_median_grayscale(tumor_seged)


value = brain_median/tumor_median


tumorless_array = original_unscaled_array.copy()

tumorless_array[mask_rescaled == 0] *= 0.5

tumorless_array[tumor_mask_rescaled > 0] = median

write_images(tumorless_array, output_image_path_Seg)
...
circled = circle_highlighted(original_unscaled_array, mask, 0.8)

print(circled.shape)


write_images(mask, output_image_path)



#write_images(original_array, output_image_path)
