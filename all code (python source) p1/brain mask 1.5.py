...
brain_seg_model_precise1 = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Brain Seg 4/Model 4"
...
brain_seg_model_precise3 = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Brain Seg 4/Model 1"
...
output_image_path_Seg3 = "/home/jiangl/Documents/python/ct to tumor identifier project/image ct  visualizations/brain 2"
...
brain_seg = keras.models.load_model(brain_seg_model_precise2)

new_array = original_array/(median/0.05)

brain_mask1 = brain_seg.predict(new_array)

binary_brain_area1 = binarize(brain_mask1, 0.8)



brain_seged = original_array.copy()

brain_seged[binary_brain_area1 == 0] = 0


brain_seg = keras.models.load_model(brain_seg_model_precise3)

new_array = original_array/(median/0.2)

brain_mask1 = brain_seg.predict(new_array)

binary_brain_area3 = binarize(brain_mask1, 0.9)




#binary_brain_area3 = combine_white_binary(binary_brain_area32, binary_brain_area31)

brain_seg = keras.models.load_model(brain_seg_model_precise1)

new_array = original_array/(median/0.05)

brain_mask1 = brain_seg.predict(new_array)

binary_brain_area2 = binarize(brain_mask1, 0.6)


brain_mask_area12 = combine_white_binary(binary_brain_area2, binary_brain_area3)

brain_mask_area1 = combine_zeros(brain_mask_area12, binary_brain_area1)

brain_mask_area = dilate_up(brain_mask_area1, blank, 1)


brain_seged = original_array.copy()

brain_seged[brain_mask_area1 == 0] = 0
...
brain_seged[brain_mask_area == 0] = 0
...
binary_brain_area2 = binarize(brain_mask1, 0.3)

binary_brain_area2 = combine_zeros(binary_brain_area2, brain_mask_area1)



binary_brain_precise = combine_white_binary(binary_brain_area2, precise_tumor_binary)



print("backsampling...")

mask_upscaled = skimage.transform.rescale(np.squeeze(brain_mask_area1.copy()[0], axis = 3), (backscale_factor, backscale_factor, backscale_factor))
...
original_unscaled_array = original_unscaled_array.copy()
...
non_tumor_brain = combine_zeros(non_tumor,original_unscaled_array)

adapted = adaptive_threshold(non_tumor_brain, 45, 45, 1)

adapted = adapted/255

threshed = binarize(original_unscaled_array, 0.05)

adapted = combine_zeros(adapted, threshed)


adapted = combine_white_binary(adapted, precise_tumor_binary_rescaled)



brain1 = dilate_up(brain_mask_precise_rescaled, blank, 1)

brain_mask = combine_zeros(brain_mask_precise_rescaled, adapted)

brain_mask = kill_small_islands(brain_mask, 1)


circled1 = circle_highlighted(original_unscaled_array, brain_mask, 0.8)
circled2 = circle_highlighted(original_unscaled_array, adapted, 0.8)
circled3 = circle_highlighted(original_unscaled_array, brain_mask, 0.8)
circled4 = circle_highlighted(original_unscaled_array, brain_mask_precise_rescaled, 0.8)
#circled2 = circle_highlighted(original_array, binary_brain_precise, 0.8)


write_images(circled1, output_image_path)
write_images(circled2, output_image_path_Seg)
write_images(circled3, output_image_path_Seg2)
write_images(circled4, output_image_path_Seg3)

brain.generate_stl(brain_mask.T, "/home/jiangl/Documents/python/ct to tumor identifier project/3d stl ct visualizations", "brain new.stl", 1)
