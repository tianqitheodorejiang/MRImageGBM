...
def kill_small_islands(array, denoise_iterations):
    ...
    markers[eroded_3d == 0] = -1
    flat = markers.ravel()
    binc = np.bincount(flat[flat != -1])
    noise_idx = np.where(binc != np.max(binc))
    mask = np.isin(markers, noise_idx)
    eroded_3d[mask] = 0
...
file = get_folder_path("/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/Brain-Tumor-Progression", 12, "flair")
...
adapted = adaptive_threshold(array, 105,105)

gray = binarize(array, 0.1)


brain_seg = keras.models.load_model(brain_seg_model)

mask = brain_seg.predict(array)

binary = binarize(mask, 0.1)

dilate = dilate_up(binary, original_array, 1)

invert = brain.invert(dilate)

array[binary == 0] = 0

com = combine_zeros(invert, adapted)

com = combine_zeros(gray, com)

killed = kill_small_islands(com, 1)

dilate = dilate_up(killed, original_array, 2)

invert =  brain.invert(dilate)

com = combine_zeros(gray, invert)

com = combine_zeros(com, adapted)

circled = circle_highlighted(original_array, com, 1)


write_images(com, output_image_path_Seg)


'''
binary = binarize(array, 0.2)

array[binary == 0] = 0

array = kill_small_islands(array, 2)

dilate = dilate_up(array, original_array, 2)
...
precise_prediction = precise_model.predict(array)
area_prediction = area_model.predict(array)
...
correct = touching_island(binary_pre, binary_area)
