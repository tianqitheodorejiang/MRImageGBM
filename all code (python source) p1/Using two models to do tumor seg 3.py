...
class highlight_ct:
    ...
    def find_median_grayscale(self, array):
        ...
        zero_pixels = float(np.count_nonzero(array<))
...
binary_brain = binarize(brain_mask, 0.2)
...
adapted = adaptive_threshold(brain_seg_array, 7, 25)



white_threshed = brain.erode_down(white, 2)

white_threshed = combine_zeros(white_threshed, binary_brain)
...
print("mode: " + str(brain.find_mode_grayscale(brain_seg_array)))


precise_model = keras.models.load_model(area_model_path)

precise_prediction = precise_model.predict(original_array)

precise_prediction[brain_seg_array < 0.1] = 0

precise_tumor_binary = binarize(precise_prediction, 0.1)

white_threshed = combine_white_binary(white_threshed, precise_tumor_binary)

correct = touching_island(precise_tumor_binary, white_threshed)

dilate = dilate_up(correct, original_array, 3)

#actual_tumor = combine_zeros(dilate, adapted)
