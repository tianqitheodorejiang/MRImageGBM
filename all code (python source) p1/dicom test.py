...
print("asdfffffffffffffffffffffffffffffffffffffffffffffffffff")
...
image_data_original = image_data/np.max(image_data)

z_zoom = image_size/image_data.shape[0]
y_zoom = image_size/image_data.shape[1]
x_zoom = image_size/image_data.shape[2]

image_data1 = skimage.transform.rescale(image_data_original, (z_zoom, y_zoom, x_zoom))
...
image_data = np.stack([np.stack([image_data_original], axis = 3)])
...
original_array = original_array1.copy()
...
prob = ext.run(original_array) 

# mask can be obtained as:
mask = prob > 0.5

'''brain_cut = original_array.copy()

brain_cut[mask  == 0] = 0


brain_median = find_median_grayscale(brain_cut)

new_array = original_array/(brain_median/50)

print(mask.shape)
prob = ext.run(new_array) 
...
circled = circle_highlighted(original_array, mask, 0.8)
