...
area_model_path = "/home/jiangl/Documents/python/ct to tumor identifier project/code files/Saved Models/Model 43"
...
#original_array = original_array/(np.max(blurred)*4)
...
brain_mask = brain_seg.predict(brain_seg_array_area)
...
brain_seg_area[binary_brain_area1 == 0] = 0

brain_seg_area = brain_seg_area/np.max(brain_seg_area)


tumor_seg = keras.models.load_model(precise_model_path)

tumor_mask1 = tumor_seg.predict(brain_seg_area)

#tumor_mask = tumor_mask/np.max(tumor_mask)

#brain_seg_area *= tumor_mask

tumor_seg = keras.models.load_model(area_model_path)

tumor_mask2 = tumor_seg.predict(brain_seg_area)

#print("max:", np.max(tumor_mask))

binary_tumor_area = binarize_max(tumor_mask2, 0.1)

binary_tumor_precise = binarize_max(tumor_mask1, 0.3)

binary_area1 = combine_white_binary(binary_tumor_precise, binary_tumor_area)

binary_area = touching_island(binary_tumor_precise, binary_area1)

binary_tumor = combine_zeros(binary_tumor_area, binary_tumor_precise)



#binary_brain_area = kill_small_islands(binary_brain_area, 1)


circled = circle_highlighted(original_array, binary_tumor_area, 1)
#circled = circle_highlighted(circled,  binary_brain_area, 0.5)



write_images(circled, output_image_path)

#brain.generate_stl(precise_prediction, "/home/jiangl/Documents/python/ct to tumor identifier project/3d stl ct visualizations", 'youre dumb.stl', 1)
