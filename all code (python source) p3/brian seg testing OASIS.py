...
def kill_small_islands(array, denoise_iterations):
    ...
    for z, image in enumerate(markers):
...
def get_files(path, img_id = "T1", mask_id = "aseg", save_folder_path = None, chunk_size = 10000000, image_size = 128):
    accs = 0
    dices = 0
    tot = 0
    images = []
    ...
    for set_num,SET in enumerate(os.listdir(path)):
        if "OAS" in SET:
            ...
            accs+=acc(mask,binary_brain_top_top_ized)
            tot+=1
            print("ACC:",accs/tot)

            actual_mask = np.stack([mask,mask],axis=-1)
            actual_mask[:] = 0
            ...
            actual_tumor = np.stack([mask,mask],axis=-1)
            actual_tumor[:] = 0

            
            actual_tumor[:,:,:,0][binary_brain_top_top_ized==0] = 1
            actual_tumor[:,:,:,1][binary_brain_top_top_ized==1] = 1

            dices+=dice(actual_mask,actual_tumor)
            print("DICE:",dices/tot)
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/data/raw ct files/Brain Seg Data Oasis 2/Actual Data",
