...
def split_train_val_test(set_):
    total = len(set_)
    train_end_val_beginning = round(0.9 * total)
    val_end_test_beginning = round(0.95 * total)
...
def dilate_up_binary(array, size):
    ...
    return np.array(blew_up > 0,dtype = "float32")
    
def get_files(path, img_id = "original array", mask_id = "rough brain", save_folder_path = None, chunk_size = 10000000, image_size = 128, bad_indexes = []):
    images = []
    masks = []
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/t1 t2 flair arrays",
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/t1 to flair arrays/Images and Masks For Tumor Segmentation",
              bad_indexes = ([3,7,34]))
