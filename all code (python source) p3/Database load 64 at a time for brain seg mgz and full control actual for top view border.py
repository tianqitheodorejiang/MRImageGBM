...
def get_files(path, img_id = "T1", mask_id = "aseg", save_folder_path = None, chunk_size = 64, image_size = 128):
    ...
    for set_num,SET in enumerate(os.listdir(path)):
        if "OAS" in SET:
            ...
            brain_mask_upscaled = skimage.transform.rescale(mask, (backscale_factor, backscale_factor, backscale_factor))
            ...
            adapted_for_brain = adaptive_threshold(np.squeeze(original_unscaled_array.copy()[0], axis = 3)/np.max(original_unscaled_array), 101, 45, 1, stacked = False)

            #brain_mask_rescaled = combine_zeros([brain_mask_rescaled, adapted_for_brain])


            generate_stl(brain_mask_rescaled.T, folder_output+"/skin_skull.stl", 1)
