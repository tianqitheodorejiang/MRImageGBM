        ...
        for n,set_ in enumerate(os.listdir(path)):
            ...
            try:
                if int(set_) not in bad_indexes:
                    ...
                    h5f.close()
                    print("\n\n",image.shape,"\n\n")
                    stoopid = image[:,:,:,3]#/np.max(image[:,:,:,3])
                    image = np.stack([image[:,:,:,0],image[:,:,:,1],image[:,:,:,2],stoopid,image[:,:,:,4],image[:,:,:,5],image[:,:,:,6]],axis=-1)
...
if data ==  main:
    get_files(["C:/Users/JiangQin/Documents/data/raw ct files/Brain-Tumor-Progression/loaded arrays for growth prediction"],
              save_folder_path = "C:/Users/JiangQin/Documents/data/raw ct files/Brain-Tumor-Progression/Images and Masks for growth prediction")
