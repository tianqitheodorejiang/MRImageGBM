...
def ConvNet(x,y,z):
    inputs = keras.layers.Input((x,y,z, 4))
    
    pool0 = inputs
    conv1, pool1 = down_block(pool0, 16, 0.1) #128 -> 64
    print(pool1.shape)
    conv2, pool2 = down_block(pool1, 32, 0.1) #64 -> 32
    conv3, pool3 = down_block(pool2, 64, 0.2) #32 -> 16
    conv4, pool4 = down_block(pool3, 128, 0.3) #16->8
    flat = tf.keras.layers.Flatten()(pool4)
    dense = tf.keras.layers.Dense(256)(flat)
...
output_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/QIN GBM Treatment Response/loaded arrays"
sets = []

model_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/treatment 1/Model 25.h5"

model = ConvNet(128,128,128)
...
for Set in range(start, len(sets)):
    try:
        ...
        if Set in backwards_indexes:
            complete_image = np.stack([new_image[:,:,:,0],new_image[:,:,:,1],old_image[:,:,:,0],old_image[:,:,:,1]],axis = -1)
            print("\n\n\n\nPREDICTIOM",model.predict(np.stack([complete_image]))," 1\n\n\n\nPREDICTION")
            if (model.predict(np.stack([complete_image])) > 0.5) == True:
                print("correct")
                correct += 1
            else:
                print("wrong")
            ...
            complete_image = np.stack([old_image[:,:,:,0],old_image[:,:,:,1],new_image[:,:,:,0],new_image[:,:,:,1]],axis = -1)
            print("\n\n\n\nPREDICTIOM",model.predict(np.stack([complete_image]))," 0\n\n\n\nPREDICTION")
            if (model.predict(np.stack([complete_image])) > 0.5) == False:
                print("correct")
                correct += 1
            else:
                print("wrong")
