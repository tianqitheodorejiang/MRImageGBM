...
output_path = "C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/loaded arrays treatment response DSC and FMISO"
                    ...
                    if flair is not None and t1 is not None and t2 is not None:                
                        ...
                        for i in range(0,1):
                            try:
                                ...
                                ia=1
                                lines = list(csv.reader(open(os.path.join(clinical_data_path,str("M"+str(ia))+".csv"))))
                                headers = lines[0]
                                ...
                                if int(date) == data[:,date_ind][sets.tolist().index(set_num)]:
                                    ...
                                    try:
                                        new_date = int(date)
                                        lines = list(csv.reader(open(os.path.join(clinical_data_path,str("DS.csv")))))
                                        headers = lines[0]
                                        survival_ind = headers.index("DSe5d")
                                        ...
                                        days = data[:,survival_ind][sets.tolist().index(set_num)]

                                        
                                        lines = list(csv.reader(open(os.path.join(clinical_data_path,str("A0.csv")))))
                                        headers = lines[0]
                                        age_ind = headers.index("entryage")
                                        ...
                                        age = data[:,age_ind][sets.tolist().index(set_num)]
                                        if days !=0:
                                            found=True
                                            current_time = i
                                            break
                                    ...
                                    if os.path.isdir(scan_path_old):
                                        ...
                                        if flair_old is not None and t1_old is not None and t2_old is not None:
                                            ...
                                            if int(older_date) == int(date):
                                                print(new_date,older_date)
                                                datasets.append([[scan_path_old + "/" + flair_old,scan_path_old + "/" + t1_old,scan_path_old + "/" + t2_old],
                                                                 [scan_path + "/" + flair,scan_path + "/" + t1,scan_path + "/" + t2],age, new_date-older_date, days])
                                                break
                            except Exception as e:
                                print(e)
                                pass
...
input_path = "C:/Users/JiangQin/Documents/data/raw ct files/ACRIN-FMISO-Brain"
path = "C:/Users/JiangQin/Documents/data/raw ct files/ACRIN-FMISO-Brain/Clinical Data/ACRIN FMISO Brain 6684 TCIA Anonymized"
    ...
    for set_ in os.listdir(input_path):
        ...
        try:
            ...
            for scan in os.listdir(set_path):
                ...
                if os.path.isdir(scan_path):
                    for mri in os.listdir(scan_path):
                        if "t2" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower()  and "flair" not in mri.lower() and os.path.isdir(scan_path + "/" + mri):
                            if t2!=None:
                                bru+=1
                            t2 = mri
                        if "t1" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower() and os.path.isdir(scan_path + "/" + mri):
                            if t1!=None:
                                bru+=1
                            t1 = mri
                        if "flair" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower()  and os.path.isdir(scan_path + "/" + mri):
                            if flair!=None:
                                bru+=1
                            flair = mri
                    if flair is not None and t1 is not None and t2 is not None:
                        ...
                        data = []
                        blub = open(os.path.join(clinical_data_path,str("V2.csv"))).read()
                        lines = blub.split("\n")
                        headers = lines[0].split(",")
                        #print(headers)
                        date_ind = headers.index("V2e5d")
                        #print(headers)
                        ...
                        #print(sets)
                        if int(date) == data[:,date_ind][sets.tolist().index(set_num)]:
                            new_date = int(data[:,date_ind][sets.tolist().index(set_num)])
                            print("new date:",new_date)
                            data = []
                            blub = open(os.path.join(clinical_data_path,str("DS.csv"))).read()
                            lines = blub.split("\n")
                            headers = lines[0].split(",")
                            survival_ind = headers.index("DSe4d")
                            ...
                            data = np.stack(data)
                            print("uhh")
                            if data[:,survival_ind][sets.tolist().index(set_num)] != 0:
                                found=True
                                days = int(data[:,survival_ind][sets.tolist().index(set_num)])
                        ...
                        if found:
                            try:
                                ...
                                lines = list(csv.reader(open(os.path.join(clinical_data_path,str("V1.csv")))))
                                headers = lines[0]
                                date_ind = headers.index("V1e2d")
                                ...
                                dates = data[:,date_ind]
                                print(sets)
                                older_date = dates[sets.tolist().index(set_num)]
                                print("older date:",older_date, new_date)
                                for scan in os.listdir(set_path):
                                    flair_old = None
                                    t1_old = None
                                    t2_old = None
                                    ...
                                    if os.path.isdir(scan_path_old):
                                        for mri in os.listdir(scan_path_old):
                                            if "t2" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower()  and "flair" not in mri.lower() and os.path.isdir(scan_path_old + "/" + mri):
                                                if t2_old!=None:
                                                    bru+=1
                                                t2_old = mri
                                            elif "t1" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower()  and os.path.isdir(scan_path_old + "/" + mri):
                                                if t1_old!=None:
                                                    bru+=1
                                                t1_old = mri
                                            elif "flair" in mri.lower() and "cor" not in mri.lower() and "sag" not in mri.lower() and os.path.isdir(scan_path_old + "/" + mri):
                                                if flair_old!=None:
                                                    bru+=1
                                                flair_old = mri
                                        if flair_old is not None and t1_old is not None and t2_old is not None:
                                            date = dicom.read_file(scan_path_old + "/" + flair_old+"/"+os.listdir(scan_path_old + "/" + flair_old)[0]).ClinicalTrialTimePointID
                                            if int(older_date) == int(date):
                                                print(new_date,older_date, date,set_num)
                                                datasets.append([[scan_path_old + "/" + flair_old,scan_path_old + "/" + t1_old,scan_path_old + "/" + t2_old],
                                                                 [scan_path + "/" + flair,scan_path + "/" + t1,scan_path + "/" + t2], new_date-older_date, days])
                                                break
                            except Exception as e:
                                printe()
                                pass
...
#sets2 = load_sets(input_path,path)
#for thing in sets2:
#    sets.append(thing)
...
for Set in range(start, len(sets)):
    ...
    print(sets[Set])
    print("blub:",sets[Set][-2])
    flair,t1,t2 = preprocess_data_set(sets[Set][0])
    ...
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image_full")
    ...
    #write_images(binary_brain_final_combined1, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/imageblub")
    ...
    write_images(binary_brain_final_combined, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/imageblub")
    ...
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image_brain")
    ...
    write_images(only_brain, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/imagebraib")
    ...
    write_images(tumor_colored/3, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image_seg")
    ...
    print("saved mask\n")
    write_images(image[:,:,:,0], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image1")
    write_images(image[:,:,:,1], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image2")
    write_images(image[:,:,:,2], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"/image3")
    ...
    flair,t1,t2 = preprocess_data_set(sets[Set][1])
    ...
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path)
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image_full")
    ...
    write_images(binary_brain_final_combined, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/imageblub")
    ...
    t2 /=(find_median_grayscale(t2)/0.2)
    flair /=(find_median_grayscale(flair)/0.2)
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image_brain")
    ...
    write_images(only_brain, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/imagebraib")
    ...
    print(np.max(tumor_colored))
    tumor_colored = np.round(tumor_colored*3)
    write_images(tumor_colored/3, output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image_seg")
    tumor_colored[tumor_binary == 0] = 0
    tumor_colored = skimage.transform.rescale(tumor_colored/3, (0.5,0.5,0.5))
    print(np.max(tumor_colored))
    tumor_colored = np.round(tumor_colored*3)
    ...
    interval = np.zeros(image[:,:,:,5].shape)
    interval[:] = sets[Set][-2]

    age = np.zeros(image[:,:,:,5].shape)
    age[:] = sets[Set][-3]
    ...
    print("saved mask\n")
    write_images(image[:,:,:,0], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image1")
    write_images(image[:,:,:,1], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image2")
    write_images(image[:,:,:,2], output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/image3")
    open(output_image_path+"/treatment response DSC FMISO/"+str(Set)+"b/response","w+").write(str(response))

    #-----------------------------------------------------------------------------------------------------------------------------------------


    complete_image = np.stack([old_image[:,:,:,0],old_image[:,:,:,1],old_image[:,:,:,2],old_image[:,:,:,3],old_image[:,:,:,4],old_image[:,:,:,5],
                               new_image[:,:,:,0],new_image[:,:,:,1],new_image[:,:,:,2],new_image[:,:,:,3],new_image[:,:,:,4],new_image[:,:,:,5],age, interval],axis = -1)
