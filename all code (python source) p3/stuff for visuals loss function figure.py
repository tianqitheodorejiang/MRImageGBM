...
for Set in range(start, len(sets)):
    ...
    tumor_mask_old = tumor_mask.copy()[0]
    tumor_mask_old = skimage.transform.rescale(tumor_mask_old, (4.0,4.0,4.0,1))
    ...
    tumor_mask = tumor_seg_channeled.predict(np.stack([only_brain]))
    tumor_mask_new = tumor_mask.copy()[0]
    tumor_mask_new = skimage.transform.rescale(tumor_mask_new, (4.0,4.0,4.0,1))
    print(tumor_mask.shape)
    print(np.max(tumor_mask))
    ...
    write_images(tumor_mask_new[:,:,:,1:], output_image_path+"/tumor growth pred/"+str(Set)+"b/imagetumorwosubtract")
    write_images(tumor_mask_old[:,:,:,1:], output_image_path+"/tumor growth pred/"+str(Set)+"b/imagetumorwosubtract1")
    write_images(np.abs(tumor_mask_new[:,:,:,1:]-tumor_mask_old[:,:,:,1:]), output_image_path+"/tumor growth pred/"+str(Set)+"b/imagetumorsubtract")
