...
brain_seg_model_top = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 34 top view/Model 22.h5"
...
brain_seg_model_side = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 30 side view/Model 72.h5"


array_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/t1 t2 flair arrays"

output_image_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/t1 to flair bright 2"
output_image_path_actual = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/t1 to flair true 2"
...
def write_images(array, test_folder_path, stacked = False):
    if not stacked:
        for n,image in enumerate(array):
            ##finds the index of the corresponding file name in the original input path from the resize factor after resampling
            file_name = str(str(n) +'.png')
...
def circle_highlighted(reference, binary, color, stacked = False):
    if not stacked:
...
def kill_small_islands(array, denoise_iterations, stack = True):
    if stacked:
        binary = np.squeeze(array.copy()[0], axis = 3)
        masked = np.squeeze(array.copy()[0], axis = 3)
    else:
...
def cut_neck(array, mask):
    ...
    for n, Slice in enumerate(mask):
        if not np.max(Slice) == 0:
            ...
            break
        wo_neck[n] = 0
    return wo_neck, mask, cut_point


def get_files(path, start_index = 0, finish_index = False, img_id = "flair", save_folder_path = None, chunk_size = 64, image_size = 128):
    start_time = time.time()
    ...
    for set_ in os.listdir(path):
        for folder in os.listdir(os.path.join(path, set_)):
            ...
            if "t2" in folder.lower():
                t2s.append(os.path.join(os.path.join(path, set_), folder))
                
    if not finish_index:
        finish_index = len(t1s)
    for i in range(start_index, finish_index):
        ...
        original_array_t1 = np.stack([np.stack([original_array1], axis = 3)])
        ...
        brain_mask1 = brain_seg.predict(original_array_t1)

        binary_brain_area = binarize(brain_mask1, 0.9)

        brain_seg_area_final = original_array_t1.copy()
        ...
        new_array = original_array_t1/(median/0.3)

        brain_mask1 = brain_seg.predict(new_array)

        binary_brain_final_combined = np.squeeze(binarize(brain_mask1[0], 0.5),axis=-1)



        original_array_t1 = np.squeeze(original_array_t1[0],axis=-1)
        ...
        bounds_finder = adaptive_threshold(bounds_finder, 101, 101, 1, stacked = False)

        bounds_finder[bounds_finder < 0.1] = 0
        bounds_finder[bounds_finder > 0] = 1
        


        bounds_finder = biggest_island(bounds_finder, stacked = False)

        #bounds_finder = kill_small_islands(bounds_finder, 1, stacked = False)
        ...
        original_array_flair = original_array1.copy()

        original_array_flair, binary_brain_final_combined,cut_neck_translation = cut_neck(original_array_flair, binary_brain_final_combined)
        original_array_flair = translate_3d(original_array_flair, [-cut_neck_translation,0,0])




        brain = highlight_ct(t2_path)
        print('initialized')
        brain.load_scan()
        print('loaded scans')
        pixel = brain.generate_pixel_data()
        ...
        original_array_t2 = original_array1.copy()

        original_array_t2, binary_brain_final_combined,cut_neck_translation = cut_neck(original_array_t2, binary_brain_final_combined)
        original_array_t2 = translate_3d(original_array_t2, [-cut_neck_translation,0,0])
        ...
        binary_brain_final_combined = translate_3d(binary_brain_final_combined, [-cut_neck_translation,0,0])

        binary_brain_final_combined = biggest_island(binary_brain_final_combined, stacked = False)
        

        only_brain = original_array_flair.copy()

        only_brain[binary_brain_final_combined == 0] = 0
        ...
        save_array(np.stack([original_array_flair,original_array_t1,original_array_t2],axis=-1), array_output_path + "/original array.h5")
        save_array(binary_brain_final_combined, array_output_path + "/rough brain seg.h5")

        circled1 = circle_highlighted(original_array_flair/(median_brain/0.35), binary_brain_final_combined, 1)
        circled2 = circle_highlighted(original_array_flair, binary_brain_final_combined, 1)


        if not os.path.exists((output_image_path + "/" + str(i))):
            os.makedirs((output_image_path + "/" + str(i)))
        else:
            shutil.rmtree((output_image_path + "/" + str(i)))
            os.makedirs((output_image_path + "/" + str(i)))
            

        write_images(circled1, (output_image_path + "/" + str(i)))


        if not os.path.exists((output_image_path_actual + "/" + str(i))):
            os.makedirs((output_image_path_actual + "/" + str(i)))
        else:
            shutil.rmtree((output_image_path_actual + "/" + str(i)))
            os.makedirs((output_image_path_actual + "/" + str(i)))
            

        write_images(circled2, (output_image_path_actual + "/" + str(i)))
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/tumor progression with only flair t1 and t2", img_id = "flair", 
              save_folder_path = "/home/jiangl/Documents/python/ct to tumor identifier project/raw ct files/TCGA GBM DATA/Images and Masks For Tumor Segmentation")
