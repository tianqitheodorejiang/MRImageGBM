...
output_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/brain border 2"
...
def get_files(path, img_id = "T1", mask_id = "aseg", save_folder_path = None, chunk_size = 64, image_size = 128):
    ...
    for set_num,SET in enumerate(os.listdir(path)):
        if "OAS" in SET:
            ...
            brain_median = random.randint(25, 40)
            ...
            brain_seg = ConvNet(128,128,128)

            brain_seg.load_weights(model_path)

            brain_mask1 = brain_seg.predict(np.stack([np.stack([t1], axis = 3)]))

            binary_brain_area = binarize(brain_mask1, 0.99)

            brain_seg_area_final = t1.copy()

            brain_seg_area_final[np.squeeze(binary_brain_area.copy()[0], axis = 3) == 0] = 0
            ...
            new_array = np.stack([np.stack([t1], axis = 3)])/(median/0.2)
            ...
            circled = circle_highlighted(only_brain/(median/0.35), mask, 0.8)
            ...
            write_images(circled, folder_output)

            images.append(only_brain)
