...
def get_files(pathes, img_id = "original array", mask_id = "rough brain", save_folder_path = None, chunk_size = 1280000, image_size = 128, bad_indexes = []):
        ...
        for n,set_ in enumerate(os.listdir(path)):
            ...
            try:
                if int(set_) not in bad_indexes:
                    ...
                    image = h5f['all_data'][:]
                    stoppid = image[:,:,:,4]/np.max(image[:,:,:,4])
                    print(np.max(stoopid))
                    h5f.close()
                    print("\n\n",image.shape,"\n\n")
                    print("min and max:",np.min(image),np.max(image))
                    ...
                    for i in range(0,4):
                        ...
                        write_images(altered[:,:,:,1], "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/treatment eval image viewing/"+str(Set)+str(i)+"/2")
...
if data ==  main:
    get_files(["C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/loaded arrays treatment response",
               "C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/loaded arrays treatment response DSC and FMISO"],
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/QIN GBM Treatment Response/Images and Masks For Tumor Segmentation survival")
