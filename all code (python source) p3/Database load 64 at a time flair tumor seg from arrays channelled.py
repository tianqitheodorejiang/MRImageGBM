    ...
    for j in range(0,int(num_files/chunk_size)+1):
        ...
        for i in range(set_size*j,set_size*(j+1)):
            try:
                ...
                #image[np.stack([mask,mask,mask],axis = -1) == 0] = 0
                ...
                print(image.shape)
                #circled = circle_highlighted(image, mask, 1)


                write_images(image, output_image_path)
                
                h5f.close()
                channelled = np.stack([mask,mask,mask,mask], axis = 3)
                channelled[:] = 0
                for z,image in enumerate(mask):
                    for y,line in enumerate(image):
                        for x,pixel in enumerate(line):
                            if pixel==0:
                                channelled[z][y][x][0] = 1
                            elif pixel==1:
                                channelled[z][y][x][1] = 1
                            elif pixel==2:
                                channelled[z][y][x][2] = 1
                            elif pixel==3:
                                channelled[z][y][x][3] = 1
                            elif pixel==4:
                                channelled[z][y][x][3] = 1
                print("mask unique:",np.unique(channelled))
                masks.append(channelled)
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays",
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays/Images and Masks For Tumor Segmentation channeled")
