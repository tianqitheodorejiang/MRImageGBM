...
brain_seg_model_area = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Model 34"
brain_seg_model_precise1 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 9/Model 102.h5"
brain_seg_model_precise2 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 25/Model 31.h5"
brain_seg_model_precise3 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 22/Model 22.h5"
brain_seg_model_precise4 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg 4/Model 1"
brain_seg_model_precise5 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS 6/Model 91.h5"
...
model_path2 = "C:/Users/JiangQin/Documents/c++/build-MRImage3D-Desktop_Qt_5_15_0_MSVC2015_64bit-Debug/models/t1_brain_top.h5"
edges_model_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/Brain Seg OASIS edges 37 top view/Model 42 (1).h5"

output_image_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/Brain Seg OASIS 36 top view image 2 52(win) translated with edges precise"
output_image_path_Seg = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/brain 1"
output_image_path_Seg2 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/brain 1 (2)"
output_image_path_Seg3 = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/brain 2"
...
class highlight_ct:
    ...
    def load_scan(self):
        ...
        ##sets the slice thickness 
        try:
            slice_thickness = raw_data[0].SliceThickness
        except:
            slice_thickness = np.abs(raw_data[0].SliceLocation - raw_data[1].SliceLocation)
    ...
    def generate_stl(self, array_3d, stl_file_path, name, stl_resolution):
        ...
        verts, faces, norm, val = marching_cubes(transposed, 0.1, step_size = stl_resolution, allow_degenerate=True)
...
def ConvNetEdges(x,y,z):
    ...
    u1 = up_block(bn, c3, 128, 0.3) #8 -> 16
    u2 = up_block(u1, c2, 64, 0.2) #16 -> 32
    u3 = up_block(u2, c1, 32, 0.1) #32 -> 64
...
path= "C:/Users/JiangQin/Documents/data/raw ct files/TCGA and Tumor progression combined"
...
for set_ in os.listdir(path):
    for folder in os.listdir(os.path.join(path, set_)):
        if "t1" in folder.lower() and "flair" not in folder.lower():
            t1s.append(os.path.join(os.path.join(path, set_), folder))
        if "flair" in folder.lower():
            flairs.append(os.path.join(os.path.join(path, set_), folder))
 
#model_path = models_path + "/" + model
image_size = 128
index = 0
#file = get_folder_path("C:/Users/JiangQin/Documents/data/raw ct files/Brain-Tumor-Progression", index, "t1prereg")
file = "C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/QIN-GBM-TR-02/05-17-1995-INVESTIGATORSCED-06224/47.000000-T1 Axial Post-85181"
model = "1"

brain = highlight_ct(file)#t1s[index])
print('initialized')
brain.load_scan()
print('loaded scans')
pixel = brain.generate_pixel_data()
print('generated pixel array')
image_data = brain.resample_array()#.T

#image_data = np.rot90(image_data, axes = (1,2))
#image_data = np.rot90(image_data, axes = (1,0))

image_data = image_data/np.max(image_data)

#image_data = scipy.ndimage.rotate(image_data, 30,axes = (2,0))
...
#original_array1 = np.rot90(original_array1, axes = (1,0))
#original_array1 = np.rot90(original_array1, axes = (0,1))
...
#brain_seg = load_model(model_path)

brain_mask1 = brain_seg.predict(original_array)

binary_brain_area = binarize(brain_mask1, 0.9)
...
#brain_seg = load_model(model_path2)

new_array = original_array/(median/0.3)

brain_mask1 = brain_seg.predict(new_array)

binary_brain_area1 = binarize(brain_mask1, 0.5)

'''only_brain = original_array.copy()

only_brain[binary_brain_area1 == 0] = 0


only_brain/=(median/0.2)

contrasted = original_array.copy()

contrasted[binary_brain_area1 == 1] = 0

counting = contrasted.copy()
counting[:] = 0
counting[contrasted > 0.1] = 1

mean_skull = np.sum(contrasted)/np.sum(counting)
print("mean skull: ",mean_skull)

contrasted/=(mean_skull/0.7)

contrasted += only_brain
print(contrasted.shape)

#contrasted = blur(contrasted, 0.2)

if not os.path.exists((output_image_path + "/" + model + "_contrsted")):
    os.makedirs((output_image_path + "/" + model + "_contrsted"))

write_images(contrasted, (output_image_path + "/" + model + "_contrsted"))
...
brain_mask1 = brain_seg.predict(contrasted)
...
print("only brain min and max: ",np.min(only_brain),np.max(only_brain))
...
brain_seg = ConvNetEdges(128,128,128)

brain_seg.load_weights(edges_model_path)
...
binary_brain_area1 = np.squeeze(binary_brain_area1.copy()[0], axis = 3)


 
mask_upscaled = skimage.transform.rescale(binary_brain_area1, (backscale_factor, backscale_factor, backscale_factor))
...
brain.resampled_array = original_unscaled_array.copy()

blank = original_unscaled_array.copy()
...
mask_upscaled = skimage.transform.rescale(np.squeeze(brain_mask1.copy()[0], axis = 3), (backscale_factor, backscale_factor, backscale_factor))
...
original_mask = np.stack([np.stack([mask_unscaled], axis = 3)])

both_mask_and_prediction = stick_at_end(brain_mask_precise_rescaled, original_mask)

brain_mask_rough_rescaled = kill_small_islands(brain_mask_precise_rescaled,2)

brain_mask_precise_rescaled = combine_zeros(brain_mask_precise_rescaled,brain_mask_rough_rescaled)

mask = blur(np.squeeze(brain_mask_precise_rescaled.copy()[0], axis = 3), 1)

brain.generate_stl(np.stack([np.stack([mask], axis = 3)]).T, ("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/3d stl ct visualizations/" + os.path.basename(output_image_path)), model + ".stl", 1)


circled = circle_highlighted(original_unscaled_array/(median/0.35), brain_mask_precise_rescaled, 1)

only_brain = original_unscaled_array.copy()/(median/0.35)

only_brain[brain_mask_precise_rescaled < 0.5] = 0

both_mask_and_prediction = stick_at_end(circled, only_brain)


if not os.path.exists((output_image_path + "/" + model)):
    os.makedirs((output_image_path + "/" + model))
'''else:
    shutil.rmtree((output_image_path + "/" + model))
    os.makedirs((output_image_path + "/" + model))'''
    
write_images(both_mask_and_prediction, (output_image_path + "/" + model))

#brain.generate_stl(brain_mask_precise_rescaled.T, ("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/3d stl ct visualizations/" + os.path.basename(output_image_path)), model + ".stl", 1)
