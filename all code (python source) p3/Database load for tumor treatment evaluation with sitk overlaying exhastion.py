...
def register(reference, final):
    R = sitk.ImageRegistrationMethod()
    R.SetMetricAsMeanSquares()
    R.SetOptimizerAsRegularStepGradientDescent(4.0, .01, 200 )
    R.SetInitialTransform(sitk.TranslationTransform(reference.GetDimension()))
    R.SetInterpolator(sitk.sitkLinear)

    R.AddCommand( sitk.sitkIterationEvent, lambda: command_iteration(R) )

    outTx = R.Execute(reference, final)
    print(outTx)
    ...
    out = resampler.Execute(final)
    simg1 = sitk.Cast(sitk.RescaleIntensity(reference), sitk.sitkUInt8)
...
for Set in range(start, len(sets)):
    ...
    t1 = register(flair_image,t1_image)
    t1[flair == 0] = 0
    print(t1.shape)
    print("loaded t1")
    write_images((t1/np.max(t1)), output_image_path+"/asdf1")
    cont = input()
    ...
    write_images((t2/np.max(t2)), output_image_path+"/asdf2")
    cont = input()
