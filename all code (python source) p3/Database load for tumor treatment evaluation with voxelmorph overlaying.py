...
# project
sys.path.append("C:/Users/JiangQin/AppData/Local/Programs/Python/Python36/Lib/site-packages/voxelmorph_legacy/ext/neuron")
from voxelmorph_legacy.src import networks, losses
import voxelmorph_legacy.ext.neuron.neuron.layers as nrn_layers
...
def preprocess_data(path):
    ...
    z_zoom = 160/image_data.shape[0]
    y_zoom = 192/image_data.shape[1]
    x_zoom = 224/image_data.shape[2]
    ...
    rescale_factor = (160*0.9)/max_size
...
def register_with_pathes(moving, fixed):
    '''# GPU handling
    gpu_id = "gpu 0"
    gpu = '/gpu:' + str(gpu_id)
    ...
    net = keras.models.load_model("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/code files/Saved models/voxelmorph/cvpr2018_vm2_cc.h5", custom_objects=custom_objects)
...
def register(fixed, moving, temp_path):
    fixed = nib.Nifti1Image(fixed, np.eye(4))
    fixed.header.get_xyzt_units()
    fixed.to_filename(temp_path + "/fixed.nii.gz")
    
    moving = nib.Nifti1Image(moving, np.eye(4))
    moving.header.get_xyzt_units()
    moving.to_filename(temp_path + "/moving.nii.gz")

    moved = register_with_pathes(temp_path + "/moving.nii.gz",temp_path + "/fixed.nii.gz")
    
    return moved

output_image_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/treatment eval testing"
...
for Set in range(start, len(sets)):
    ...
    t1 = np.squeeze(register(flair, t1, output_image_path)[0],axis = -1)
    print(t1.shape)
    #t1[flair == 0] = 0

    write_images((t1/np.max(t1)), output_image_path)
    ...
    t2 = register(np.array(flair>0.1,dtype="float64"), np.array(t2>0.1,dtype="float64"), output_image_path)
    print(t2.shape)
    t2 = np.squeeze(t2[0],axis = -1)
    #t2[flair == 0] = 0
    print(t2.shape)
    print("loaded t2")
