...
output_path = "C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/loaded arrays treatment response"


input_path = "C:/Users/JiangQin/Documents/data/raw ct files/IvyGAP"
path = "C:/Users/JiangQin/Documents/data/raw ct files/IvyGAP/Clinical Data"
...
def load_sets(input_path,clinical_data_path,datasets=[]):
    ...
    for set_ in os.listdir(input_path):
        ...
        scan_dates = []
        try:
            lines = list(csv.reader(open(os.path.join(clinical_data_path,str("tumor_details.csv")))))
            headers = lines[0]
            days_ind = headers.index("survival_days")
            age_ind = headers.index("age_in_years")
            del lines[0]

            print(set_)
            #print(np.stack(lines)[:,1].tolist())
            names = np.stack(lines)[:,1]
            names = [nm[:nm.index("-")] for nm in names]
            print(names)
            set_ind = names.index(set_)
            age_str = lines[set_ind][age_ind]
            age = int(age_str[:age_str.index(" ")])

            
            lines = np.stack(lines)
            data = np.zeros(lines.shape)
                ...
                for scan in os.listdir(set_path):
                    ...
                    if os.path.isdir(scan_path):
                        for mri in os.listdir(scan_path):
                            if "t2" in mri.lower() and "sag" not in mri.lower() and "gre" not in mri.lower() and "flair" not in mri.lower() and os.path.isdir(scan_path + "/" + mri):
                                if t2!=None:
                                    bru+=1
                                t2 = mri
                        ...
                        if flair is not None and t1 is not None and t2 is not None:                
                            ...
                            scan_dates.append(int(date))
                            scans.append([scan_path + "/" + flair,scan_path + "/" + t1,scan_path + "/" + t2])
                scans = [x for _, x in sorted(zip(scan_dates,scans), key=lambda pair: pair[0])]
                scan_dates.sort()
                print(scan_dates)
                found = False
                for n, date in enumerate(scan_dates):
                    if date<0 and scan_dates[n+1]>=0 and scan_dates[n+2]>=10:
                        print(date,scan_dates[n+1],scan_dates[n+2])
                        old_date = n+1
                        new_date = n+2
                        found = True
                        ...
                        new_date = n+1
                        found = True
                print(dicom.read_file(scans[new_date][0]+"/"+os.listdir(scan_path + "/" + flair)[0]).ClinicalTrialTimePointID)
                if int(dicom.read_file(scans[new_date][0]+"/"+os.listdir(scan_path + "/" + flair)[0]).ClinicalTrialTimePointID)<100 and found:
                    print("\n\n")
                    datasets.append([[scans[old_date][0],scans[old_date][1],scans[old_date][2]],[scans[new_date][0],scans[new_date][1],scans[new_date][2]],age,scan_dates[new_date]-scan_dates[old_date],days])
...
for Set in range(start, len(sets)):
    ...
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response testing/"+str(Set)+"/image_full")
    ...
    #write_images(binary_brain_final_combined1, output_image_path+"/treatment response testing/"+str(Set)+"/imageblub")
    ...
    write_images(binary_brain_final_combined, output_image_path+"/treatment response testing/"+str(Set)+"/imageblub")
    ...
    write_images(cflair, output_image_path+"/treatment response testing/"+str(Set)+"/bran1")
    write_images(ct1, output_image_path+"/treatment response testing/"+str(Set)+"/bran2")
    write_images(ct2, output_image_path+"/treatment response testing/"+str(Set)+"/bran3")

    write_images(bflair, output_image_path+"/treatment response testing/"+str(Set)+"/bbran1")
    write_images(bt1, output_image_path+"/treatment response testing/"+str(Set)+"/bbran2")
    write_images(bt2, output_image_path+"/treatment response testing/"+str(Set)+"/bbran3")
    ...
    write_images(np.stack([flair,t1,t2],axis=-1), output_image_path+"/treatment response testing/"+str(Set)+"/image_brain")
    ...
    write_images(only_brain, output_image_path+"/treatment response testing/"+str(Set)+"/imagebraib")
    ...
    write_images(tumor_colored/3, output_image_path+"/treatment response testing/"+str(Set)+"/image_seg")
    '''tumor_colored[tumor_binary == 0] = 0
    tumor_colored-=1
    tumor_colored[tumor_binary == 1] += 1
    tumor_colored[tumor_colored<0] = 0'''
    write_images(tumor_colored/3, output_image_path+"/treatment response testing/"+str(Set)+"/image_seg_bin")
    ...
    write_images(tumor_mask[:,:,:,1], output_image_path+"/treatment response testing/"+str(Set)+"/tum1")
    write_images(tumor_mask[:,:,:,2], output_image_path+"/treatment response testing/"+str(Set)+"/tum2")
    write_images(tumor_mask[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"/tum3")
    write_images(tumor_mask[:,:,:,1]+tumor_mask[:,:,:,2]+tumor_mask[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"/tum4")
    write_images(np.stack([tumor_mask[:,:,:,1],tumor_mask[:,:,:,2],tumor_mask[:,:,:,3]],axis=-1), output_image_path+"/treatment response testing/"+str(Set)+"b/tum5")
    ...
    write_images(cflair, output_image_path+"/treatment response testing/"+str(Set)+"/bran1a")
    write_images(ct1, output_image_path+"/treatment response testing/"+str(Set)+"/bran2a")
    write_images(ct2, output_image_path+"/treatment response testing/"+str(Set)+"/bran3a")


    write_images(bflair, output_image_path+"/treatment response testing/"+str(Set)+"/bbran1a")
    write_images(bt1, output_image_path+"/treatment response testing/"+str(Set)+"/bbran2a")
    write_images(bt2, output_image_path+"/treatment response testing/"+str(Set)+"/bbran3a")
    ...
    print("saved mask\n")
    write_images(image[:,:,:,0], output_image_path+"/treatment response testing/"+str(Set)+"/image1")
    write_images(image[:,:,:,1], output_image_path+"/treatment response testing/"+str(Set)+"/image2")
    write_images(image[:,:,:,2], output_image_path+"/treatment response testing/"+str(Set)+"/image3")
    write_images(image[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"/image4")'''
    ...
    write_images(cflair, output_image_path+"/treatment response testing/"+str(Set)+"b/bran1")
    write_images(ct1, output_image_path+"/treatment response testing/"+str(Set)+"b/bran2")
    write_images(ct2, output_image_path+"/treatment response testing/"+str(Set)+"b/bran3")

    write_images(bflair, output_image_path+"/treatment response testing/"+str(Set)+"b/bbran1")
    write_images(bt1, output_image_path+"/treatment response testing/"+str(Set)+"b/bbran2")
    write_images(bt2, output_image_path+"/treatment response testing/"+str(Set)+"b/bbran3")
    ...
    tumor_mask[:,:,:,1]-=tumor_mask[:,:,:,2]
    tumor_mask[:,:,:,1]-=tumor_mask[:,:,:,3]'''
    write_images(tumor_mask[:,:,:,1], output_image_path+"/treatment response testing/"+str(Set)+"b/tum1")
    write_images(tumor_mask[:,:,:,2], output_image_path+"/treatment response testing/"+str(Set)+"b/tum2")
    write_images(tumor_mask[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"b/tum3")
    write_images(tumor_mask[:,:,:,1]+tumor_mask[:,:,:,2]+tumor_mask[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"b/tum4")
    write_images(np.stack([tumor_mask[:,:,:,1],tumor_mask[:,:,:,2],tumor_mask[:,:,:,3]],axis=-1), output_image_path+"/treatment response testing/"+str(Set)+"b/tum5")
    ...
    write_images(cflair, output_image_path+"/treatment response testing/"+str(Set)+"b/bran1a")
    write_images(ct1, output_image_path+"/treatment response testing/"+str(Set)+"b/bran2a")
    write_images(ct2, output_image_path+"/treatment response testing/"+str(Set)+"b/bran3a")
    ...
    write_images(image[:,:,:,3], output_image_path+"/treatment response testing/"+str(Set)+"/image4")
    print("saved mask\n")
    write_images(image[:,:,:,0], output_image_path+"/treatment response testing/"+str(Set)+"b/image1")
    write_images(image[:,:,:,1], output_image_path+"/treatment response testing/"+str(Set)+"b/image2")
    write_images(image[:,:,:,2], output_image_path+"/treatment response testing/"+str(Set)+"b/image3")
