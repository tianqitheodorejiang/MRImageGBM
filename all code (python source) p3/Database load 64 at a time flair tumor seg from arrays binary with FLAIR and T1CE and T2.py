    ...
    for j in range(0,int(num_files/chunk_size)+1):
        ...
        for i in range(set_size*j,set_size*(j+1)):
            try:
                ...
                channeled = np.stack([mask,mask],axis=-1)
                channeled[:] = 0
                channeled[:,:,:,0][mask == 0] = 1
                channeled[:,:,:,1][mask == 1] = 1

                channeled[:,:,:,0][mask == 0] = 1
                
                write_images(np.argmax(channeled,axis = -1), output_image_path)
                
                masks.append(channeled)
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays",
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/flair tumor seg arrays/Images and Masks For Tumor Segmentation binary channeled")
