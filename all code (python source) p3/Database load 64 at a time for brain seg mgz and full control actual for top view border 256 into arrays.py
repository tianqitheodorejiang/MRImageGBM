...
output_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/raw ct files/Brain Seg Data Oasis top view/Actual Data/loaded arrays"
...
def get_files(path, img_id = "T1", mask_id = "aseg", save_folder_path = None, chunk_size = 64, image_size = 128):
    ...
    for set_num,SET in enumerate(os.listdir(path)):
        if "OAS" in SET and set_num >= start:
            ...
            brain_mask_top = brain_seg.predict(np.stack([np.stack([t1], axis = 3)]))
            ...
            binary_brain_wo_median_combined = combine_zeros([binary_brain_top_top_ized])
            ...
            new_array_top = np.stack([np.stack([t1], axis = 3)])/(median/0.3)
            ...
            t1[binary_brain_top_top_ized == 0] = 0

            folder_output = output_path + "/" + str(set_num)

            save_array(t1, folder_output + "/image.h5")

            write_images(binary_brain_top_top_ized, folder_output)
            ...
            #write_images(only_brain*2, folder_output)
            
            #masks.append(mask)

            save_array(mask, folder_output + "/mask.h5")
