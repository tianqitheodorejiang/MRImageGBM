...
def get_files(path, img_id = "original array", mask_id = "rough brain", save_folder_path = None, chunk_size = 1280000, image_size = 128, bad_indexes = []):
    ...
    for n,set_ in enumerate(os.listdir(path)):
        ...
        try:
            if int(set_) not in bad_indexes:
                ...
                if np.argmax(mask)==3 or np.argmax(mask) == 1:
                    amt = 6
                elif np.argmax(mask)==0:
                    amt = 3
                else:
                    amt = 1
                ...
                for i in range(0,amt):
                    rotate_amt = random.randint(-10,10)
                    axes = [(0,1),(1,0),(2,1),(1,2),(2,0),(0,2)]
                    axis = axes[random.randint(0,len(axes)-1)]
                    print("blub")

                    rotates_image = []
                    for j in range(0,image.shape[-1]):
                        rotates_image.append(scipy.ndimage.rotate(image[:,:,:,j],rotate_amt,axes = axis,reshape = False))
                    altered = np.stack(rotates_image,axis=-1)
                    print("shape",altered.shape)
                    
                    altered[:,:,:,0] *= (random.randint(7,10)/10)
                    altered[:,:,:,1] *= (random.randint(7,10)/10)
                    altered[:,:,:,2] *= (random.randint(7,10)/10)
                    altered[:,:,:,6] *= (random.randint(7,10)/10)
                    altered[:,:,:,7] *= (random.randint(7,10)/10)
                    altered[:,:,:,8] *= (random.randint(7,10)/10)

                    altered = np.stack([altered[:,:,:,3],altered[:,:,:,4],altered[:,:,:,5],altered[:,:,:,9],altered[:,:,:,10],altered[:,:,:,11]],axis=-1)
                    print(str(Set)+str(i))
                    print(altered.shape)

                    write_images(altered[:,:,:,0], "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/treatment eval image viewing/"+str(Set)+str(i)+"/1")
                    write_images(altered[:,:,:,3], "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/treatment eval image viewing/"+str(Set)+str(i)+"/2")
                ...
                for i in range(0,amt):
                    ...
                    print(mask)
                    masks.append(mask)
                    open("C:/Users/JiangQin/Documents/python/ct to tumor identifier project/image ct  visualizations/Machine Learning 2 models test/treatment eval image viewing/"+str(Set)+str(i)+"/mask.txt","w+").write(str(mask))

                print("PROGRESSIONS",progressions)
...
if data ==  main:
    get_files("C:/Users/JiangQin/Documents/data/raw ct files/QIN GBM Treatment Response/loaded arrays 2",
              save_folder_path = "C:/Users/JiangQin/Documents/python/ct to tumor identifier project/QIN GBM Treatment Response/Images and Masks For Tumor Segmentation")
